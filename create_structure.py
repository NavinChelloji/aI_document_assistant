...
def create_structure_from_text(text, root_dir_name):
    ...
    for line in lines:
        ...
        depth = line_no_comment.rfind(name)
        
        if depth == 0 and len(path_stack) == 1 and path_stack[0][0] == -1:
            path_stack = [(0, root_dir_name)]
            continue
            
        while len(path_stack) > 1 and path_stack[-1][0] >= depth:
            path_stack.pop()
...
def extract_tree_block(filepath, section_title):
    ...
    # Find the first ```text after the section
