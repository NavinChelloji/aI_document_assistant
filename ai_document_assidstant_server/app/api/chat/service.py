...
async def create_chat_session(db: AsyncSession, session_in: ChatSessionCreate, user_id: UUID) -> ChatSession:
...
async def get_chat_sessions(db: AsyncSession, workspace_id: UUID, user_id: UUID) -> List[ChatSession]:
...
async def append_message_to_session(db: AsyncSession, session_id: UUID, message_in: ChatMessageCreate, user_id: UUID) -> ChatMessage:
    ...
    query_embedding = embeddings_model.embed_query(message_in.question)
    ...
    try:
        ...
        results = collection.query(
            ...
            n_results=3,
            where={"workspace_id": str(session.workspace_id)}
        )
    ...
    if context_text:
        augmented_prompt = (
            ...
            f"Context:\n{context_text}\n\n"
            f"Question: {message_in.question}"
        )
    else:
        augmented_prompt = message_in.question
    # -----------------------------------
    ...
    chat_msg = ChatMessage(
        ...
        model="ollama", # Update if dynamic
...
async def get_session_messages(db: AsyncSession, session_id: UUID, user_id: UUID) -> List[ChatMessage]:
    result = await db.execute(select(ChatSession).where(ChatSession.id == session_id))
    session = result.scalars().first()
    if not session:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Chat session not found")
